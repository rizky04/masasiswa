...
@section('page_title', 'Edit Hasil Uji')
...
<div class="card shadow-sm">
  <div class="card-body">
    ...
    <form id="formHasilUji" action="{{ route('pengujian.update', $analisa->id_pcanalisa) }}" method="POST" enctype="multipart/form-data">
      @csrf
      @method('PUT')
        ...
        <div class="card-body bg-light">
          <div class="form-row">
            <div class="form-group col-md-6">
              ...
              <textarea name="contoh_uji" id="contoh_uji" class="form-control editor" rows="5">{{ $analisa->contohUji->contoh_uji ?? '' }}</textarea>
            </div>
            <div class="form-group col-md-6">
              <label for="image"><strong>Upload Gambar</strong></label>
              <input type="file" name="image" id="image" class="form-control">
              @if($analisa->contohUji->image)
                <img src="{{ asset($analisa->contohUji->image) }}" alt="Gambar Contoh Uji" class="mt-2" style="max-width:150px;">
              @endif
              <input type="hidden" name="id_pcanalisa" value="{{ $analisa->id_pcanalisa }}">
            </div>
          </div>
        </div>
      </div>
      ...
      <div id="dataTeknisWrapper">
        ...
        <div class="form-row data-teknis-item mb-3 p-3 border rounded bg-light">
          <div class="form-group col-md-5">
            ...
            <input type="text" name="subject_uji[]" class="form-control" value="{{ $item->subject_teknis_kemasan }}" required>
          </div>
          <div class="form-group col-md-6">
            <label>Content Subject Uji Kemasan</label>
            <textarea name="content_uji[]" class="form-control editor">{{ $item->nama_teknis_kemasan }}</textarea>
          </div>
      ...
      <div id="dataIdentifikasiWrapper">
        ...
        <div class="form-row data-identifikasi-item mb-3 p-3 border rounded bg-light">
          <div class="form-group col-md-5">
            ...
            <input type="text" name="nama_teknis_identifikasi[]" class="form-control" value="{{ $item->nama_teknis_identifikasi }}" required>
          </div>
          <div class="form-group col-md-6">
            <label>Subject Teknis Identifikasi</label>
            <textarea name="subject_teknis_identifikasi[]" class="form-control editor">{{ $item->subject_teknis_identifikasi }}</textarea>
          </div>
          <div class="form-group col-md-1 d-flex align-items-center justify-content-center">
            <button type="button" class="btn btn-outline-danger btn-sm btn-remove"><i class="fas fa-trash"></i></button>
          </div>
        ...
        <div class="form-row program-uji-item mb-3 p-3 border rounded bg-light">
          <div class="form-group col-md-3">
            ...
            <select name="jenis_program_uji[]" class="form-control" required>
              ...
              <option value="Uji Jatuh" {{ $item->jenis_program_uji=='Uji Jatuh' ? 'selected' : '' }}>Uji Jatuh</option>
              <option value="Uji Kebocoran" {{ $item->jenis_program_uji=='Uji Kebocoran' ? 'selected' : '' }}>Uji Kebocoran</option>
              <option value="Uji Tekanan Dalam" {{ $item->jenis_program_uji=='Uji Tekanan Dalam' ? 'selected' : '' }}>Uji Tekanan Dalam</option>
              <option value="Uji Tumpuk" {{ $item->jenis_program_uji=='Uji Tumpuk' ? 'selected' : '' }}>Uji Tumpuk</option>
            </select>
          </div>
          <div class="form-group col-md-4">
            <label>Nama Program Uji</label>
            <input type="text" name="nama_program_uji[]" class="form-control" value="{{ $item->nama_program_uji }}" required>
          </div>
          <div class="form-group col-md-4">
            <label>Subject Program Uji</label>
            <textarea name="subject_program_uji[]" class="form-control editor">{{ $item->subject_program_uji }}</textarea>
          </div>
          <div class="form-group col-md-1 d-flex align-items-center justify-content-center">
            <button type="button" class="btn btn-outline-danger btn-sm btn-remove"><i class="fas fa-trash"></i></button>
          </div>
      ...
      <div id="hasilUjiWrapper">
        ...
        <div class="form-row hasil-uji-item mb-3 p-3 border rounded bg-light">
          ...
          <div class="form-group col-md-8">
            ...
            <input type="text" name="nama_hasil_uji[]" class="form-control" value="{{ $item->nama_hasil_uji }}" required>
          </div>
          <div class="form-group col-md-1 d-flex align-items-center justify-content-center">
            <button type="button" class="btn btn-outline-danger btn-sm btn-remove"><i class="fas fa-trash"></i></button>
          </div>
      ...
      <div class="form-group">
        ...
        <textarea name="kesimpulan_uji" class="form-control editor" rows="5">{{ $analisa->kesimpulanUji->first()->kesimpulan_uji ?? '' }}</textarea>
      </div>

      <div class="text-right mt-4">
        <button type="submit" class="btn btn-success">💾 Update Hasil</button>
      </div>
    </form>
  </div>
</div>
...
// Template untuk tambah data
const sections = [
  {btn: '#addDataTeknis', wrap: '#dataTeknisWrapper', html: `
    <div class="form-row data-teknis-item mb-3 p-3 border rounded bg-light">
      <div class="form-group col-md-5">
        <label>Subject Uji Kemasan</label>
  ...
  `},
  {btn: '#addDataIdentifikasi', wrap: '#dataIdentifikasiWrapper', html: `
    <div class="form-row data-identifikasi-item mb-3 p-3 border rounded bg-light">
      <div class="form-group col-md-5">
        <label>Nama Teknis Identifikasi</label>
  ...
  `},
  {btn: '#addProgramUji', wrap: '#programUjiWrapper', html: `
    <div class="form-row program-uji-item mb-3 p-3 border rounded bg-light">
      <div class="form-group col-md-3">
        <label>Jenis Program Uji</label>
        <select name="jenis_program_uji[]" class="form-control" required>
  ...
  `},
  {btn: '#addHasilUji', wrap: '#hasilUjiWrapper', html: `
    <div class="form-row hasil-uji-item mb-3 p-3 border rounded bg-light">
      <div class="form-group col-md-3">
        <label>Jenis Program Uji</label>
        <select name="jenis_program_uji_hasil[]" class="form-control" required>
...
sections.forEach(sec => {
  $(sec.btn).on('click', function () {
    ...
    initEditors(); // Reinit CKEditor untuk input baru
  });
});
...
// Dynamic add/remove fields (sama seperti sebelumnya)
const sections = [
  {btn: '#addDataTeknis', wrap: '#dataTeknisWrapper', html: `...`}, // isi sama seperti sebelumnya
  {btn: '#addDataIdentifikasi', wrap: '#dataIdentifikasiWrapper', html: `...`},
  {btn: '#addProgramUji', wrap: '#programUjiWrapper', html: `...`},
  {btn: '#addHasilUji', wrap: '#hasilUjiWrapper', html: `...`}
];
